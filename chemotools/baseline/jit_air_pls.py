...
class JITAirPls(OneToOneFeatureMixin, BaseEstimator, TransformerMixin):
    """
    ...
    def transform(self, X: np.ndarray, y=None) -> np.ndarray:
        ...
        for i, x in enumerate(X_):
            X_[i] = x - _calculate_air_pls(x, self.nr_iterations, self.polynomial_order, self.lam)

        return X_.reshape(-1, 1) if X_.ndim == 1 else X_

@nb.njit
def _calculate_whittaker_smooth(x, w, polynomial_order, lam):
    X = np.matrix(x)
    m = X.size
    E = eye(m, format="csc")
...
@nb.njit
def _calculate_air_pls(x, nr_iterations, polynomial_order, lam):
    m = x.shape[0]
    w = np.ones(m)

    for i in range(1, nr_iterations):
        z = _calculate_whittaker_smooth(x, w, polynomial_order, lam)
        d = x - z
